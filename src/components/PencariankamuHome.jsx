  ...
  return (
    <div className="bg-white mt-12 shadow-md rounded-xl">
      <div className="h-16 p-4 shadow-md flex flex-row items-center gap-2">
        <IoSearch className="text-4xl text-Primary hover:text-blue-800" />
        <h1 className="font-bold text-2xl">Berdasarkan Pencarianmu</h1>
      </div>

      <div className="flex flex-wrap justify-center gap-6 p-6">
        {produk.slice(10,15).map((product) => {
          const potonganJudul = potongTeks(product.title, 30);
          ...
          return (
            ...
            >
              <div>
                ...
                <p className="title text-sm mt-2" title={product.title}>{potonganJudul}</p>
                <p className="price text-sm font-bold">Rp{(product.price * 16000).toLocaleString('id-ID')}</p>
